<template>
  <div class='sea'>
    <mt-search id="search"  @click.native='toSearch'
      cancel-text="取消"
      placeholder="请输入要咨询的问题">
    </mt-search>
    <mt-navbar  v-model="selected">
      <mt-tab-item v-for="n in questionList" :key="n.key" :id="n.tabName">{{n.tabName}}</mt-tab-item>
    </mt-navbar>
    <mt-tab-container  v-model="selected">
      <mt-tab-container-item v-for="n in questionList" :key="n.key" :id="n.tabName">
        <!-- <mt-cell :title="n.name"></mt-cell> -->
        <mt-cell v-for="m in n.detail" :title="m.question" :key="m.id" to="/info"  @click.native="toInfo(m)"  is-link/>
...
.sea .mint-searchbar-inner .mintui-search {
  font-size: 20px;
}
.sea .mint-tab-item-label{
  font-size: 14px;
}
.sea .mint-navbar .mint-tab-item.is-selected {
  color: #2d8cf0;
